...
function getCallbackUrl()
{
	...
	return $protocol . 'localhost/Salozone/WebFiles/response2.php';
}
...
if(isset($_GET["id"]) && isset($_GET["order"])) {
	...
	$order = mysqli_query($con, "SELECT * FROM `order_items` WHERE `user_id` = '".$customer1['cust_id']."' AND `id` = '".$_GET["id"]."' AND `order_id` = '".$_GET["order"]."' AND `payment_status` = '0'");
	$count = mysqli_num_rows($order);
	if($count == 1) {
		$_SESSION['id'] = $_GET["id"];
		$_SESSION['order'] = $_GET["order"];
	$order1 = mysqli_fetch_array($order);
$total = $order1['order_tot_amt'];
$delivery = $order1['delivery_amt'];
$coupon = $order1['coupon_amt'];
$amount = $order1['gross_amt'];
						  ...
						  <form action="#" name="payment_form" id="payment_form">
							...
							<div class="row form-group">
								<div class="col-md-6">
									...
									<input type="hidden" id="key" name="key" placeholder="Merchant Key" class="form-control" value="E3IoU5Qj" />
								</div>
							</div>
							...
							<div class="row form-group">
								<div class="col-md-6">
									<input type="hidden" id="fname" name="fname" placeholder="Name" class="form-control" value="<?php echo $customer1['cust_name']; ?>" required="required" autocomplete="off" />
								</div>
								<div class="col-md-6">
									<input type="hidden" id="email" name="email" placeholder="Email ID" class="form-control" pattern="[a-z0-9._%+-]+@[a-z0-9.-]+\.[a-z]{2,4}$" title="example@example.com" required="required" autocomplete="off" value="<?php echo $customer1['cust_email']; ?>" />
								</div>
							</div>
							
							<div class="row form-group">
								<div class="col-md-12">
									<input type="hidden" id="mobile" name="mobile" placeholder="Mobile/Cell Number" class="form-control" required="required" pattern="^[6789]\d{9}$" title="Enter 10 digit valid mobile number" maxlength="10" autocomplete="off" value="<?php echo $customer1['cust_mobile']; ?>" />
								</div>
								...
								<div class="_3pUB-LDjlpx6e9ACL8E_1x _1ztQTskd2_GRzGb0OH6YwR" style="">
									...
									<?php if($coupon != '') { ?>
										<div class="hPO8BoyBkYEU2ujeCWJgk _1ovga__v3v_0DJ3-9UggBS H1Lb6afUHKzH-OTelE23x" style="font-size: 14px;">
											...
											<div class="_36rMyLl0yKiOS-udBUSM8a" style="color: rgb(117, 117, 117);">₹ <?php echo($coupon); ?></div>
										</div>
									<?php } ?>
									<div class="hPO8BoyBkYEU2ujeCWJgk _1ovga__v3v_0DJ3-9UggBS H1Lb6afUHKzH-OTelE23x" style="font-size: 14px;">
...
<?php 	} else {
			echo "<center><h3 class='text-danger'>Wrong order payment...</h3></center>";
		}
	} else {
